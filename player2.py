...
def has_chessman_on_way(pos, new_pos, direction):
    ...
    while True:
        ...
        if field.field[pos[1] + y_speed][pos[0] + x_speed] != 0:
            if field.field[pos[1] + y_speed][pos[0] + x_speed][0] != active_player and pos[1] + y_speed == new_pos[
                1] and pos[0] + x_speed == new_pos[0]:
                return "take"
            else:
                return True
        else:
...
class Horse(ChessMan):
    ...
    def is_can_move(self, new_pos):
        if (abs(new_pos[0] - self.x) == 1 and abs(new_pos[1] - self.y) == 2) or (
                abs(new_pos[0] - self.x) == 2 and abs(new_pos[1] - self.y) == 1):
...
class Field:
    ...
    def create_chessmans(self):
        if looks == "white":
            ...
            self.white_chessmans = [self.white_king, self.white_queen, self.white_castle_l, self.white_horse_l,
                                    self.white_bishop_l,
            ...
            for i in range(8):
                ...
                self.white_chessmans.append(white_pawn)
            self.black_king = King(3, 7, "black")
            self.black_queen = Queen(4, 7, "black")
            ...
            for i in range(8):
                ...
                self.black_chessmans.append(black_pawn)
            self.field[0][0] = ("white", "c")
            self.field[0][1] = ("white", "h")
            self.field[0][2] = ("white", "b")
            self.field[0][3] = ("white", "k")
            self.field[0][4] = ("white", "q")
            self.field[0][5] = ("white", "b")
            self.field[0][6] = ("white", "h")
            self.field[0][7] = ("white", "c")

            self.field[7][0] = ("black", "c")
            self.field[7][1] = ("black", "h")
            self.field[7][2] = ("black", "b")
            self.field[7][3] = ("black", "k")
            self.field[7][4] = ("black", "q")
            self.field[7][5] = ("black", "b")
            self.field[7][6] = ("black", "h")
            self.field[7][7] = ("black", "c")
            for i in range(8):
                self.field[1][i] = ("white", "p")
            for i in range(8):
                self.field[6][i] = ("black", "p")
...
def coordinates_changer_from_server(pos):
    new_x = field.width-1 - int(pos[0])
    new_y = field.height-1 - int(pos[1])
    return (new_x, new_y)
...
def events_check():
    global process_running, active_cell, active_chessman, active_player,local_last_way
                ...
                if looks == active_player:
                    ...
                    if is_correct(pos):
                        ...
                        if not active_chessman:
                            ...
                            if res:
                                url = f"{url_base}?method=SET&sender={looks}&pos={active_cell[0]};{active_cell[1]}&new_pos={new_pos[0]};{new_pos[1]}"
                                requests.request("GET", url)
                                # print("___________________________________________________________________________")
                                # res = json.loads(res.text)

                                local_last_way = [active_cell, new_pos]
                                active_chessman = None
                                active_cell = None
                                # change_player()
...
def mainloop():
    global local_last_way, active_chessman
                    ...
                    if field.field[pos[1]][pos[0]][0] == "white":
                        ...
                        for chessman in field.black_chessmans:
                            if pos[0] == chessman.x and pos[1] == chessman.y:
                                ...
                                break
                    change_player()
                    local_last_way = [pos,new_pos]
                    # for i in field.field:
                    #     print(i)
        # print(looks, active_player)
